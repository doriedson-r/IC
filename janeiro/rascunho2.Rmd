...
g1 = dados_imput %>% group_by(mes) %>% mutate(media_mensal = mean(Valor.de.Venda)) %>%
  ggplot(aes(y = media_mensal, x = mes)) +
  theme(axis.text = element_text(angle=90, size=1)) +
  theme_dark() +
  geom_point() +
  labs(x = "Data", y = "Preço (R$)", title = "Médias mensais dos preços da gasolina")
...
ar_ = forecast::Arima(serie, order = c(2, 3, 0), seasonal = c(0,0,0))
