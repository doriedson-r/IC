...
modelando = function(data, p_values=c(0:5), d_values=c(0:4), q_values=c(0:5),
                      P_values=c(0:5), D_values=c(0:4), Q_values=c(0:5)){
  ...
  Result = data.frame(p=0,d=0,q=0,P,D,Q,
...
```{r}
# Particulamente, não havia tentado o modelo SARIMA por ter pouca prática com o mesmo.
```

-   Período: 2018 a 2023;
-   Posto X;
-   Quantidade de observações iniciais: `VERIFICAR E COLOCAR AQUI`
-   Método de agrupamento: **média mensal**;
-   Quantidade de observações finais;
-   Imputação de dados na observação de , sendo a da média daquele ano;
...
aux = forecast::Arima(serie, order = c(0,1,0), seasonal = c(1,1,1))
